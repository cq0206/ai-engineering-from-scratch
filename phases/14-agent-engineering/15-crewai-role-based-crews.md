...
**前置条件：** 第 14 阶段 · 12（工作流模式）、第 14 阶段 · 14（演员模型）
**耗时：** ~75 分钟
...
- 用标准库（stdlib）实现一个三智能体团队（研究员、写作者、编辑）来产出一份简报。
...
- **Flow。** 由你掌控的事件驱动图。`@start` 标记入口。`@listen(topic)` 标记当其他步骤发出该主题（topic）时触发的步骤。每一步都是普通 Python（内部也可以调用 Crew）。适合：生产。可观测、可测试、确定性强。
...
`code/main.py` 用标准库（stdlib）实现了这两种形态，以及一个三智能体 Crew。
...
具体演示：由研究员、写作者、编辑组成的团队，围绕“2026 智能体工程”生成一份简报。研究员拉取（模拟的）资料来源，写作者起草，编辑收紧润色。随后同一个团队通过 Flow 再运行一次，以展示确定性形态。
...
执行轨迹覆盖：Sequential Crew 如何通过 `context` 串接输出；Hierarchical Crew 中管理者的选择过程（研究员、写作者、编辑，最后是 “done”）；Flow 如何通过显式主题（topic）值（`researched`、`drafted`、`edited`）运行同样三步；工具调用如何通过 `@tool` 路由；以及长期记忆如何在两次 kickoff 之间持续存在。
...
`outputs/skill-crew-or-flow.md` 会为一个任务选择 Crew 还是 Flow，并搭建最小实现。它会强制拒绝以下情况：没有背景设定的 Crew、没有显式主题（topic）的 Flow、以及专家少于三个却使用 Hierarchical 的方案。
...
6. 阅读 CrewAI 文档简介。把这个玩具示例迁移到真实的 `crewai` API。标准库版（stdlib）跳过了哪些保证？
7. 把 AgentOps 或 Langfuse（第 24 课）接到一次真实运行上。标准库版（stdlib）漏掉了哪些轨迹？

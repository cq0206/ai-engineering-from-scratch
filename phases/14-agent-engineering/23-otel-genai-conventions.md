...
> OpenTelemetry 的 GenAI SIG（于 2024 年 4 月启动）定义了智能体遥测的标准模式。span 名称、属性和内容捕获规则正在跨厂商收敛，因此在 Datadog、Grafana、Jaeger 和 Honeycomb 中，智能体追踪的含义是一致的。
...
- 区分 `invoke_agent` 的 CLIENT span 与 INTERNAL span，以及它们各自适用的场景。
- 列出顶层 GenAI 属性：提供方名称、请求模型、数据源 ID。
- 解释内容捕获契约：选择加入、`OTEL_SEMCONV_STABILITY_OPT_IN`、外部引用建议。

## 问题

每个厂商都发明自己的 span 命名方式。运维团队最后不得不为每个框架单独构建仪表盘。OpenTelemetry 的 GenAI SIG 通过定义一个全生态都可以对齐的统一标准来解决这个问题。
...
2. **Agent spans。** `create_agent`（构建智能体时）和 `invoke_agent`（运行智能体时）。
...
推荐的生产模式：将内容存储在外部（S3、你的日志存储），只在 span 上记录引用（例如外部内容 ID，而不是正文内容）。这是第 27 课中的内容投毒防御，接到了可观测性体系里。
...
- **在 span 中捕获完整提示词。** 追踪里会出现运维可读的 PII、密钥和客户数据。应改为外部存储。
- **没有 `gen_ai.provider.name`。** 缺少归因时，多提供商仪表盘会失效。
- **Span 没有父链接。** 工具 span 会变成孤儿。始终传播上下文。
...
- 一个脚本化的智能体运行，会发出：`create_agent`、`invoke_agent`（INTERNAL）、每个工具的 span，以及用于 LLM 调用的 `chat` span。
...
输出内容：一个包含所有必需 GenAI 属性的 span 树，以及一个展示选择加入内容引用的“外部存储”。
...
`outputs/skill-otel-genai.md` 会把 OTel GenAI span 接入现有智能体，并配置内容捕获默认值和外部引用存储。

## 练习

1. 为你第 01 课的 ReAct 循环添加 `invoke_agent`（INTERNAL）和每工具 span。发送到一个 Jaeger 实例。
2. 以“仅引用”模式添加内容捕获：把提示词写入 SQLite，span 属性只携带行 ID。
...
| CLIENT span | “远程调用” | 调用远程智能体服务时的 span |
...
- [OpenAI Agents SDK](https://openai.github.io/openai-agents-python/) — 默认带 GenAI spans

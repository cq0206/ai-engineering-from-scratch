...
**同一模型，只改运行框架（harness），就从 Terminal Bench 前 30 名之外跃升到前 5。** LangChain 的 *Anatomy of an Agent Harness*（2026 年 4 月）显示：一个编码智能体只通过修改运行框架，就从 Terminal Bench 2.0 的前 30 名之外跃升到第 5 名。模型没变。工作面变了。名次差了 25 位。

**Vercel 通过删除工具，把 80% 提升到 100%。** Vercel 报告称，把其智能体的工具删掉 80% 后，成功率从 80% 提升到了 100%。工具暴露面更小，范围更锐利，失败路径更少。负空间赢了。
...
结论不是“运行框架会永远赢”。模型会随着时间吸收运行框架中的技巧。真正的结论是：在今天，工程负担仍然落在这七个工作面上，而数字已经证明了这一点。
...
| 术语 | 人们常说的话 | 它真正的含义 |
|------|----------------|------------------------|
| 示例应用 | “玩具仓库” | 小，但足够真实，能覆盖全部七个工作面 |
| 流水线 | “工作流” | 智能体遵循的一组有序工作面读写序列 |
...
- 第 14 阶段 · 32 到 40 —— 本课端到端演练的这些工作面
- 第 14 阶段 · 19 —— 作为宏观基准的 SWE-bench、GAIA、AgentBench，本课对此形成补充
- 第 14 阶段 · 30 —— 同一运行框架可接入的评估驱动智能体开发
